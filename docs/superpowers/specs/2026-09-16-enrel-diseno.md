...
enrel es un modelo open source que, dado un artículo de prensa en español, devuelve sus entidades (personas, organizaciones, lugares, cargos, normas), las agrupa por identidad dentro del artículo y extrae las relaciones de poder que el texto afirma entre ellas. Está pensado para que los laboratorios de medios construyan grafos, Graph RAG, hipervínculos automáticos y asistentes internos sin depender de una API externa.
...
| Vigencia como eje propio de toda relación (vigente, pasada, futura, relativo a la fecha del artículo), ortogonal a la modalidad de ocupa_cargo (titular, aspirante) | Dejar la vigencia únicamente en el atributo estado de ocupa_cargo, con aspirante como si fuera un tiempo más | El 14 % de las relaciones del oro trae vigencia no trivial en relaciones que no son de cargo (409 pasadas y 29 futuras de 1.548, de las que 229 pasadas son de cargo): limitarla a ocupa_cargo perdía la mayor parte de esa información. Aspirar es una modalidad, no un tiempo: tratarlo como vigencia hace que «aspiró a la Presidencia y perdió» se lea como «ocupará el cargo» (candidaturas en el 25 % del archivo, 16.034 de 63.991 artículos). Por eso el eje temporal (vigencia) y el de modalidad (titular/aspirante, atributo propio de ocupa_cargo) quedan separados y ortogonales. Decisión del 2026-09-17; las clases finas quedan en 24 en ese momento (ver más abajo la corrección a 25 por impulsa_norma). |
| Tipos de entidad reducidos a 5 (persona, organizacion, lugar, cargo, norma); se retiran obra y monto | Mantener los siete tipos de legajo | Medido sobre el oro de 125 documentos: monto tiene 214 entidades y cero relaciones que lo toquen; obra tiene 75 entidades y 16 relaciones, todas en vinculo_sin_tipo, y 27 de esas 75 son secciones propias de La Silla Vacía («Detector de Mentiras»), lo que sesga el modelo hacia un solo medio. lugar se queda pese a ser puramente contextual en muchos casos: no es un hueco de anotación. Las anotaciones no se pierden, siguen en legajo; solo el exportador de enrel descarta los dos tipos. |
| Relación nueva impulsa_norma (persona, organizacion → norma), separada de apoya_a por la regla de «gana la más específica» | Que todo acto legislativo positivo cayera en apoya_a | Sobre los 63.991 artículos del corpus: 1.116 (1,74 %) contienen un acto legislativo inequívoco sobre una norma (radicarla, redactarla, ser su ponente, sacarla adelante, sancionarla, aprobarla), frente a 604 (0,94 %) con apoyo explícito y 415 (0,65 %) con oposición explícita hacia una norma. El acto pesa más que el apoyo y la oposición juntos, así que merece su propia relación en vez de diluirse en apoya_a. Hundir o archivar una ley sigue siendo se_opone_a: es un acto, pero de signo contrario, y mezclar los dos signos en una sola etiqueta no ayuda. Dieciocho relaciones tipadas más la reserva; 25 clases finas. |
| El título entra al documento como párrafo anotable, `pi = -1`, con `ini`/`fin` relativos a él; el desplazamiento de documento de una mención de título es `ini` a secas, el de una del cuerpo sigue siendo `desplazamiento_cuerpo + offsets_parrafo[pi] + ini` | Dejar el título presente en el texto pero sin etiquetar (el estado anterior); numerar el título como `pi = 0` y correr los párrafos del cuerpo | El título sin marcar no era una omisión sino un ejemplo negativo para un clasificador de tokens: enseñaba que «Petro» en un titular no es persona mientras el mismo «Petro» cuarenta caracteres más abajo sí lo era. Medido sobre el oro de 125 documentos: 94 (75 %) tenían en el título una entidad anotada en el cuerpo y sin marcar arriba —165 menciones (83 persona, 37 organizacion, 23 lugar, 17 cargo, 5 norma). En quien-es-quien es peor: en 509 de 1.344 perfiles (37,9 %) el nombre completo solo aparece en el título y no vuelve a salir en el cuerpo, y 89 arrancan sin sujeto explícito («Es exministro de Hacienda…»); sin el título el nodo del grafo se llamaría «Echeverry» en vez de «Juan Carlos Echeverry Garzón». Tampoco se podían resolver homónimos: en el registro de 935 nombres del Quién es Quién hay 297 apellidos compartidos por dos o más personas distintas («Gómez» lo llevan 27). `pi = -1` y no `pi = 0` porque renumerar habría dejado las anotaciones del cuerpo ya hechas apuntando al párrafo equivocado. Implementado en `enrel/anotacion/desde_legajo.py` (commit `a74dca5`). |
| Relación nueva estudio_en (persona → organizacion), familia A, sin atributos | Dejar la formación académica sin relación propia (cayendo en trabaja_en, miembro_de o vinculo_sin_tipo); incluir la materia estudiada como parte de la relación | En quien-es-quien, 802 de 1.344 perfiles (59,7 %) afirman formación académica; en el corpus entero, 2.745 de 63.991 artículos (4,29 %), dos veces y media lo que pesa impulsa_norma (1,74 %). No solapa con trabaja_en: Echeverry estudió Economía en la Universidad de los Andes y después fue decano de esa facultad, dos aristas hacia la misma organización y ambas ciertas. Es un suceso (`es_suceso`): graduarse no vuelve falso haber estudiado allí, igual que dejar de dirigir algo no vuelve falso haberlo fundado. La materia queda fuera a propósito: el hecho es ternario (persona, materia, institución) pero la cabeza de relaciones clasifica pares ordenados de grupos con un bilineal, binaria por construcción; además «Economía» no es un nombre propio y como nodo sería un concentrador sin poder discriminante —es una propiedad de la persona, no una arista—. Sin equivalente en FollowTheMoney, queda con «—», como impulsa_norma. Diecinueve relaciones tipadas más la reserva; 26 clases finas. |

## 3. Esquema

### 3.1 Tipos de entidad

Cinco de los siete de legajo: se retiran obra y monto, medido sobre el oro de 125 documentos. monto tiene 214 entidades marcadas y cero relaciones que lo toquen en ningún extremo: no hay nada que afirmar de una cifra. obra tiene 75 entidades y 16 relaciones, todas colapsadas a vinculo_sin_tipo; 27 de esas 75 son secciones propias de La Silla Vacía («Detector de Mentiras»), así que entrenar sobre ellas enseñaría los nombres de las secciones de un solo medio, lo contrario de lo que busca un proyecto pensado para servir a otros medios. lugar se queda a pesar de ser puramente contextual en muchos casos (Colombia, Bogotá, Estados Unidos, sin nada que afirmar): eso no es un hueco de anotación, es una entidad legítima sin relación en ese caso concreto. Las anotaciones de obra y monto no se pierden —siguen en el sqlite de legajo, que conserva sus siete tipos—; es el exportador de enrel el que descarta las dos. Se admite anidamiento entre tipos distintos («Antioquia» dentro de «Gobernador de Antioquia»); no entre el mismo tipo.
...
Diecinueve relaciones tipadas más una de reserva. Cada una tiene dominio y rango por tipo, y en la guía de anotación un párrafo de definición, tres ejemplos positivos y dos negativos cercanos en español. Las relaciones son excluyentes para un mismo par: se elige la más específica que el texto afirme.
...
| estudio_en | persona → organizacion | no | — | estudió en | — |
...
| impulsa_norma | persona, organizacion → norma | no | — | impulsa (predicado 26); «autor de» cuando el destino es una norma | — |
...
Todas las relaciones llevan además un campo `vigencia`, eje propio y no un atributo más: `vigente`, `pasada` o `futura`, por defecto `vigente`, siempre relativo a la fecha del artículo (la fecha dice cuándo se afirmó algo, no cuándo fue verdad). `futura` cubre lo anunciado («asumirá», «será»). El viejo atributo de ocupa_cargo (actual, anterior, aspirante) mezclaba dos ejes distintos: uno temporal, que ahora es esta vigencia común a las 19 relaciones; y uno de modalidad —ser titular del cargo o solo aspirar a él— que no es un tiempo y por eso sigue siendo el atributo propio de ocupa_cargo (titular, aspirante), ortogonal a la vigencia. Los cuatro cruces tienen sentido y antes no se podían decir todos: titular + vigente lo ejerce hoy; titular + pasada lo ejerció; aspirante + vigente es candidato ahora; aspirante + pasada fue candidato y ya no. En las relaciones que son sucesos y no estados —`nombro_a`, `sucedio_a`, `fundo`, `contrato_a`, `financia_a`, `impulsa_norma`, `estudio_en`, señaladas por `es_suceso(relacion)`— la vigencia rara vez aplica y normalmente se queda en `vigente`.
...
- impulsa_norma exige un acto sobre la norma —radicarla, redactarla, ser su ponente, sacarla adelante, sancionarla, aprobarla—, no solo una postura declarada; gana la más específica, la misma regla que ya separa nombro_a de apoya_a. «Radicó el proyecto», «es el ponente», «sacó adelante la reforma» son impulsa_norma; «respaldó el proyecto», «votó a favor» sin acto propio son apoya_a. Hundir o archivar una ley sigue siendo se_opone_a, aunque también es un acto: es de signo contrario y no conviene mezclar los dos signos en una sola etiqueta.
...
Clases finas del clasificador: las 19 relaciones se despliegan en 25 clases finas (ocupa_cargo ×2 modalidades, familiar_de ×4 parentescos, investigado_por ×3 etapas, las otras 16 —incluidas impulsa_norma y estudio_en— tal cual) más vinculo_sin_tipo, 26 en total. La vigencia (vigente, pasada, futura) es un eje aparte, ortogonal y común a las 26 —incluidas las dos de ocupa_cargo—, que no las multiplica por tres. Se evalúa a los dos niveles.
...
`obra` y `monto` ya no son tipos del esquema de enrel (§3.1): se retiraron por completo, no solo de las relaciones. `norma`, que sí sigue siendo un tipo, tenía el mismo problema por descuido —160 normas marcadas en el oro y ninguna relación que las tocara— y ya está corregido arriba: apoya_a, impulsa_norma y se_opone_a le dan destino. Si en una versión siguiente se quisiera recuperar el monto como algo más que contexto, lo correcto sería un atributo de financia_a y contrato_a (en FollowTheMoney el importe es una propiedad de la arista de pago, no un nodo), no una relación aparte.

**Relaciones candidatas para la v0.2.** Entran cuando cumplan el mismo criterio que publica una relación (arriba): al menos 60 ejemplos positivos y acuerdo humano ≥ 0,70, medidos al cerrar la etapa 1. `autor_de` ya no está en la lista: es lo que `impulsa_norma` resuelve, con evidencia de que la frontera con apoya_a sí se sostiene (la regla de «gana la más específica» que ya separa nombro_a de apoya_a).
...
Clasificación de tramos. Para cada tramo de 1 a 16 palabras se construye una representación a partir de los estados de la primera y la última subpalabra más una codificación de la anchura, y se clasifica en 6 clases (5 tipos más «ninguno»). Trabaja sobre palabras, no sobre subpalabras, con la partición `\w+(?:[-_]\w+)*|\S` y offsets de caracteres. Permite anidamiento entre tipos distintos. Decodificación: se aceptan los tramos con probabilidad máxima en una clase distinta de «ninguno»; entre tramos solapados del mismo tipo gana el de mayor probabilidad. Negativos: durante el entrenamiento se muestrean tramos «ninguno» en proporción fija a los positivos (inicial 8:1).
...
4. Cargos y normas solo por regla 1.
...
1. **Dirigido por relación, 2.000 artículos.** Para cada una de las 19 relaciones hay un léxico de disparadores en `enrel/corpus/disparadores.py` (verbos, sustantivos de parentesco, fórmulas: «nombró a», «reemplazó a», «fundador», «accionista», «socio de», «filial», «esposa», «financió», «licitación», «imputado», «con sede en», «respaldó», «se opone», «radicó», «ponente»). Se toman hasta 120 artículos por relación entre los que contienen al menos un disparador, priorizando los que contienen disparadores de varias relaciones. Dos fuentes con localización ya hecha se aprovechan como disparadores de máxima precisión: `alineado.jsonl` de Quién-AI para familiar_de (unas 2.350 relaciones familiares con cita literal) y `cargos.jsonl` para ocupa_cargo. Medido sobre el archivo, todas las relaciones tienen miles de candidatos salvo ubicado_en (437 con «con sede en»); estudio_en, incorporada después de esta medición, dispara en 7.223 artículos (11,3 % del archivo), así que la cuota se cumple en las 19.
...
- Familia A, cargos y trabajo: ocupa_cargo (con modalidad), nombro_a, sucedio_a, trabaja_en, dirige, miembro_de, estudio_en.
- Familia B, empresa y dinero: fundo, propietario_de, socio_de, parte_de, contrato_a, financia_a.
- Familia C, familia, política, justicia y lugar: familiar_de (con parentesco), apoya_a, impulsa_norma, se_opone_a, investigado_por (con etapa), ubicado_en.

Cada prompt lleva solo sus 6 o 7 relaciones con definición, tipos admitidos, tres positivos y dos negativos cercanos, y la lista de confusiones frecuentes con la relación correcta. Salida: `{"cabeza": id, "cola": id, "relacion", "atributo", "vigencia", "cita": subcadena literal}`. También puede devolver `vinculo_sin_tipo` cuando ve un vínculo que no encaja. Cada relación propuesta lleva además `vigencia` (vigente, pasada o futura), relativa a la fecha del artículo, que el prompt da junto con el resto de metadatos y explica junto con las marcas del texto que la señalan.
...
Exportador `enrel/anotacion/desde_legajo.py`: lee un lote de `legajo.sqlite` (ruta configurable, porque el usuario anota en el Mac o en el PC) o los JSON por artículo del formato de `oro_apartado.py`, aplica el mapeo de 3.3, convierte `cuando` en `vigencia` para todas las relaciones (vigente → vigente, pasada → pasada, futura → futura) y mapea los predicados de cargo de legajo a ocupa_cargo con su atributo y su vigencia: «ocupa el cargo» → titular con la vigencia de `cuando` (si `cuando` es futura, es un nombramiento anunciado, «será ministro», no una candidatura); «ocupó el cargo» y «renunció a» → titular con vigencia pasada; «aspira al cargo» y «aspira a» → aspirante con la vigencia de `cuando` —el mapeo también acepta los predicados viejos—, toma los grupos de `resoluciones` («misma») y de las menciones repetidas, recompone el documento entero —título más cuerpo— a partir de los párrafos con offsets globales, incluido el título como párrafo anotable `pi = -1` (`ini`/`fin` relativos al título; su desplazamiento de documento es `ini` a secas, el de un párrafo del cuerpo sigue siendo `desplazamiento_cuerpo + offsets_parrafo[pi] + ini`), valida y escribe el formato interno. Prueba de ida y vuelta sobre los 125 artículos de oro existentes.
...
6. «Impulsa» y «autor de» con destino norma se mapean a impulsa_norma, no a apoya_a: si el texto describe un acto (radicar, redactar, ser ponente, sacar adelante, sancionar, aprobar), no un simple respaldo. No gastar tiempo en «se reunió con», «citado en», «destinado a», «sanciona con» ni «demandó a»: se mapean a vinculo_sin_tipo.
